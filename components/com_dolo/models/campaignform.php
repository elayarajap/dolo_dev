 ...
 * Dolo model.
 */
class DoloModelCampaignForm extends JModelForm
{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_dolo.campaign', 'campaignform', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	{
		$data = JFactory::getApplication()->getUserState('com_dolo.edit.campaign.data', array());
        if (empty($data)) {
            $data = $this->getData();
        }
        ...
        if ($table->save($data) === true) {
        	// To check for UMS category
				...
				if($groupNames=='ums') 
				{
					...
					foreach ($data[collaborators] as $cb){

						
						$check_insert = mysql_query("SELECT count(*) FROM ".$jconfig->dbprefix."dolo_user_brand_mapping WHERE user_id='".$cb."' AND brand_id='".$data[brandid]."'");
			        	$row_count_user_brand = mysql_fetch_array($check_insert);
						$tot_count_userbrand = $row_count_user_brand[0];

			        	if($tot_count_userbrand==0)
			        	{
			        	$insert_operation = mysql_query("INSERT INTO ".$jconfig->dbprefix."dolo_user_brand_mapping VALUES ('','1','".$data[created_by]."','".$cb."','".$data[brandid]."')");
			        	}
